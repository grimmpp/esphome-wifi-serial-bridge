...
static const char *TAG = "StreamServerComponent";
...
void StreamServerComponent::write()
{
    ...
    while ((len = this->recv_buf_.size()) > 0)
    {
        ...
        ESP_LOGD(TAG, "Wrote to '%s' to UART", this->to_text_string(this->recv_buf_.data(), len).c_str());
        this->recv_buf_.erase(this->recv_buf_.begin(), this->recv_buf_.begin() + len);
    }
}

std::string StreamServerComponent::to_hex_string(const uint8_t *bytes, size_t length)
{
    // Stringstream to build the hex string
    std::stringstream hexStringStream;

    // Iterate over the byte array and append each byte as hex
    for (size_t i = 0; i < length; ++i)
    {
        hexStringStream << std::setw(2) << std::setfill('0') << std::hex << (int)bytes[i];
    }
...
std::string StreamServerComponent::to_text_string(const uint8_t *bytes, size_t length)
{
...
StreamServerComponent::Client::Client(AsyncClient *client, std::vector<uint8_t> &recv_buf) : tcp_client{client}, identifier{client->remoteIP().toString().c_str()}, disconnected{false}
{
    ...
    this->tcp_client->onData([&](void *h, AsyncClient *client, void *data, size_t len)
        ...
        recv_buf.insert(recv_buf.end(), buf, buf + len); }, nullptr);
}
